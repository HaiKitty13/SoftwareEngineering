    ...
    private fun getRecipeData(callback: (List<Recipe>) -> Unit) {
        ...
        firestore.collection("users")
            ...
            .addOnSuccessListener { documents ->
                ...
                val bookmarkedRecipeIds = document?.get("bookmark_recipes") as? List<String> ?: emptyList()
                ...
                for (recipeId in bookmarkedRecipeIds) {
                    firestore.collection("recipes")
                        ...
                        .addOnSuccessListener { recipeDoc ->
                            ...
                            completedQueries++
                            if (completedQueries == bookmarkedRecipeIds.size) {
                                // Final check sebelum callback
                                if (isAdded && context != null) {
                                    callback(resultList)
                                }
                            }
